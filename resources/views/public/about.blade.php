@extends('layouts.app', ['seo' => $seo]) @section('content') @php
    $aboutBanners = collect([setting('about_banner_1'), setting('about_banner_2')])
        ->filter()
        ->values();
    $firstBanner = $aboutBanners->first();
@endphp @if ($firstBanner) @push('styles')
    <link rel="preload" as="image" href="{{ Storage::url($firstBanner) }}" fetchpriority="high">
    @endpush @endif {{-- Banner Section --}} <section
        class="relative bg-slate-900 text-white overflow-hidden" x-data="{ current: 0, banners: @json($aboutBanners->map(fn($b) => Storage::url($b))), get hasBanners() { return this.banners.length > 0 }, timer: null, start() { if (!this.hasBanners || this.banners.length < 2) return;
                this.timer = setInterval(() => { this.current = (this.current + 1) % this.banners.length }, 5000) }, stop() { if (this.timer) { clearInterval(this.timer);
                    this.timer = null } }, goTo(i) { this.stop();
                this.current = i;
                this.start() } }" x-init="start()">
        <div class="relative h-[40vh] lg:h-[50vh] min-h-[320px] flex items-center"> {{-- Banner Images --}} <template
                x-if="hasBanners">
            ...
            </template> {{-- Fallback --}} <template x-if="!hasBanners">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                <div class="max-w-3xl" data-aos="fade-up"> <span
                        ...
                        Tentang Kami </span>
                    <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight"> Mitra Reklame <br> <span
                            class="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400">Terpercaya
                            Anda</span> </h1>
                </div>
            </div>
        </div> {{-- Banner Dots --}} <div x-show="hasBanners && banners.length > 1"
            class="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2"> <template
    ...
    </section> {{-- Company Profile --}} <section class="py-20 lg:py-28">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                <div data-aos="fade-right"> <span
                        class="text-green-600 font-semibold text-sm uppercase tracking-wider">Profil Perusahaan</span>
                    <h2 class="text-3xl lg:text-4xl font-bold text-slate-900 mt-2 mb-6 ">
                        {{ setting('site_name', 'PT. Jaya Makmur') }} </h2>
                    <div class="space-y-4 text-slate-600 leading-relaxed ">
                        <p> {{ setting('site_name', 'PT. Jaya Makmur Kreasi') }} adalah perusahaan yang bergerak di bidang
                            Advertising, Packaging, dan Warehousing </p>
                        <p> PT Jaya Makmur Kreasi adalah berkomitmen memberikan solusi profesional sesuai dengan kebutuhan
                            setiap klien. Dengan didukung oleh tim yang berpengalaman, kami menghadirkan layanan yang
                            inovatif, efisien, dan berkualitas untuk membantu meningkatkan nilai serta pertumbuhan bisnis
                            klien. </p>
                        <p> Kami percaya bahwa kemitraan jangka panjang dan kepuasan pelanggan merupakan kunci utama dalam
                            menciptakan hasil terbaik, karena bagi kami keberhasilan Anda adalah keberhasilan kami. </p>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4" data-aos="fade-left">
                    ...
                    <div class="bg-emerald-50 rounded-2xl p-6 lg:p-8 text-center">
                        <p class="text-3xl lg:text-4xl font-bold text-emerald-600">2</p>
    ...
    </section> {{-- Visi Misi --}} <section class="bg-slate-50 py-20 lg:py-28">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-14" data-aos="fade-up"> <span
                    class="text-green-600 font-semibold text-sm uppercase tracking-wider">Visi & Misi</span>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="bg-white rounded-2xl p-8 lg:p-10 border border-slate-200 shadow-sm" data-aos="fade-up">
                    <div class="w-14 h-14 bg-green-100 rounded-xl flex items-center justify-center mb-5"> <svg
                ...
                <div class="bg-white rounded-2xl p-8 lg:p-10 border border-slate-200 shadow-sm" data-aos="fade-up"
                    data-aos-delay="100">
                    <div class="w-14 h-14 bg-emerald-100 rounded-xl flex items-center justify-center mb-5"> <svg
                            class="w-7 h-7 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                        </svg> </div>
                    <h3 class="text-xl font-bold text-slate-900 mb-3">Misi</h3>
                    <ul class="space-y-3 text-slate-500">
                        <li class="flex items-start gap-2"> <svg class="w-5 h-5 text-green-500 mt-0.5 shrink-0"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4.5 12.75l6 6 9-13.5" />
                            </svg> <span><b>Kualitas Terjamin:</b> Menyediakan produk-produk berkualitas, mulai dari
                                kerajinan bambu tradisional hingga sistem dekorasi jendela modern (blinds), yang memiliki
                                daya tahan tinggi. </span> </li>
                        <li class="flex items-start gap-2"> <svg class="w-5 h-5 text-green-500 mt-0.5 shrink-0"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4.5 12.75l6 6 9-13.5" />
                            </svg> <span><b>Pelayanan Profesional:</b> Memberikan pengalaman belanja yang mudah dan
                                responsif melalui berbagai kanal digital (Marketplace dan Landing Page). </span> </li>
                        <li class="flex items-start gap-2"> <svg class="w-5 h-5 text-green-500 mt-0.5 shrink-0"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4.5 12.75l6 6 9-13.5" />
                            </svg> <span><b>Dukungan Produk Lokal:</b> Memberdayakan potensi kerajinan lokal (seperti besek
                                dan perabotan kayu) untuk bersaing di pasar modern.</span> </li>
                        <li class="flex items-start gap-2"> <svg class="w-5 h-5 text-green-500 mt-0.5 shrink-0"
                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4.5 12.75l6 6 9-13.5" />
                            </svg> <span><b>Inovasi Layanan:</b> Terus memperbarui sistem pemesanan dan pembayaran agar
                                lebih praktis dan aman bagi pembeli. </span> </li>
                    </ul>
                </div>
            </div>
        </div>
    </section> {{-- Why Choose Us --}} <section class="py-20 lg:py-28">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-14" data-aos="fade-up"> <span
                    class="text-green-600 font-semibold text-sm uppercase tracking-wider">Keunggulan</span>
    ...
    </section> {{-- Latest Portfolios --}} @if ($latestPortfolios->isNotEmpty())
        <section class="bg-slate-50 py-20 lg:py-28">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10" data-aos="fade-up">
                    <div> <span class="text-green-600 font-semibold text-sm uppercase tracking-wider">Karya Terbaru</span>
                        ...
                        <article data-aos="fade-up" data-aos-delay="{{ $index * 100 }}"
                            ...
                            <div class="p-5">
                                <h3 class="font-bold text-slate-900 text-lg leading-tight mb-1"> <a
                                        ...
                                        class="hover:text-green-600 transition-colors">{{ $portfolio->title }}</a> </h3>
