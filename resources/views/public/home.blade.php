@extends('layouts.app', ['seo' => $seo]) @section('content') @php
    $heroBanners = collect([setting('home_banner_1'), setting('home_banner_2'), setting('home_banner_3')])
        ->filter()
        ->values();
    $firstBanner = $heroBanners->first();
@endphp @if ($firstBanner) @push('styles')
    <link rel="preload" as="image" href="{{ Storage::url($firstBanner) }}" fetchpriority="high">
    @endpush @endif {{-- Hero / Banner Carousel --}} <section
        class="relative bg-slate-900 text-white min-h-screen flex items-center overflow-hidden" x-data="{
            current: 0,
            banners: {{ \Illuminate\Support\Js::from($heroBanners->map(fn($b) => Storage::url($b))) }},
            get hasBanners() { return this.banners.length > 0 },
            autoplayTimer: null,
            init() { if (this.hasBanners && this.banners.length > 1) { this.startAutoplay(); } },
            startAutoplay() {
                this.stopAutoplay();
                this.autoplayTimer = setInterval(() => { this.next(false); }, 5000);
            },
            stopAutoplay() {
            ...
            next(reset = true) { this.current = (this.current + 1) % this.banners.length; if (reset) this.startAutoplay(); },
            prev() {
                this.current = (this.current - 1 + this.banners.length) % this.banners.length;
                this.startAutoplay();
            }
        }">
        {{-- Banner Images --}} <template x-if="hasBanners">
        ...
        </template> {{-- Fallback decorative background --}} <template x-if="!hasBanners">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32 lg:py-40 w-full">
            <div class="max-w-3xl" data-aos="fade-up"> <span
                    ...
                    <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span> Solusi Reklame Profesional </span>
                <h1 class="text-4xl sm:text-5xl lg:text-7xl font-extrabold leading-[1.1] mb-6"> Solusi Reklame <br> <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-emerald-400">Terbaik untuk
                        Brand</span> Anda </h1>
                <p class="text-lg sm:text-xl text-slate-300 mb-10 max-w-2xl leading-relaxed"> Jadikan bisnis Anda lebih
                    dikenal dengan billboard dan media reklame premium di lokasi strategis seluruh Indonesia. </p>
                <div class="flex flex-wrap gap-4"> <a href="{{ route('catalog.index') }}"
                        ...
                        Jelajahi Katalog <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                        </svg> </a> <a href="{{ route('inquiry.create') }}"
        ...
        </div> {{-- Banner Navigation Dots --}} <div x-show="hasBanners && banners.length > 1"
            class="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2"> <template
                x-for="(banner, i) in banners" :key="i"> <button @click="goTo(i)"
                    class="w-2.5 h-2.5 rounded-full transition-all duration-300"
                    :class="current === i ? 'bg-white w-8' : 'bg-white/40 hover:bg-white/60'"
                    :aria-label="'Slide ' + (i + 1)"></button> </template> </div>
        {{-- Navigation Arrows --}} <div x-show="hasBanners && banners.length > 1"
            class="absolute inset-x-0 top-1/2 -translate-y-1/2 z-10 flex items-center justify-between px-4 lg:px-8"> <button
                @click="prev()" name="previous" aria-label="Previous slide"
                ...
                </svg> </button> <button @click="next()" name="next" aria-label="Next slide"
                class="p-2 lg:p-3 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white rounded-full transition-all hover:scale-110">
                <svg class="w-5 h-5 lg:w-6 lg:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                </svg> </button> </div>
        <div class="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-slate-50 to-transparent"></div>
    </section> {{-- Stats / Counters --}} <section class="relative -mt-20 z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
            ...
            <div class="text-center">
                <p class="text-3xl lg:text-4xl font-bold text-green-600">2</p>
                <p class="text-sm text-slate-500 mt-1">Area Layanan</p>
            </div>
            <div class="text-center">
    ...
    </section> {{-- How It Works --}} <section class="py-20 lg:py-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-14" data-aos="fade-up"> <span
                class="text-green-600 font-semibold text-sm uppercase tracking-wider">Bagaimana Cara Kerjanya</span>
            <h2 class="section-title mt-2 ">Mudah & Cepat</h2>
            <p class="section-subtitle mx-auto text-slate-500 ">Dapatkan titik reklame impian Anda dalam beberapa langkah
                mudah.</p>
        </div>
    ...
    </section> {{-- Featured Points --}} @if ($featuredPoints->isNotEmpty())
        <section class="bg-slate-50 py-20 lg:py-28">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10" data-aos="fade-up">
                    <div> <span class="text-green-600 font-semibold text-sm uppercase tracking-wider">Populer</span>
                    ...
                    </div> <a href="{{ route('catalog.index') }}"
    ...
    @endif {{-- Why Choose Us --}} <section class="py-20 lg:py-28">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-14" data-aos="fade-up"> <span
                    class="text-green-600 font-semibold text-sm uppercase tracking-wider">Keunggulan Kami</span>
                <h2 class="section-title mt-2 ">Mengapa Memilih Kami?</h2>
                <p class="section-subtitle mx-auto text-slate-500 ">Kami berkomitmen memberikan solusi reklame terbaik
                    untuk bisnis Anda. </p>
            </div>
    ...
    </section> {{-- Testimonials --}} <section class="bg-slate-900 py-20 lg:py-28 relative overflow-hidden">
        <div class="absolute inset-0 opacity-10">
            <div class="absolute top-0 left-1/4 w-64 h-64 bg-green-500 rounded-full blur-[100px]"></div>
        </div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative" x-data="{ active: 0, testimonials: [{ name: 'Budi Santoso', role: 'CEO, PT. Maju Jaya', text: 'Pelayanan sangat profesional! Billboard kami terpasang tepat waktu dan hasilnya melebihi ekspektasi. Trafik ke toko meningkat signifikan.' }, { name: 'Siti Rahmawati', role: 'Marketing Manager, Fashion Brand', text: 'Lokasi reklame yang ditawarkan sangat strategis. Tim Jaya Makmur membantu kami memilih titik terbaik untuk brand kami.' }, { name: 'Andi Wijaya', role: 'Owner, Resto Group', text: 'Sudah 3 tahun bekerja sama dan tidak pernah kecewa. Proses cepat, harga kompetitif, dan hasil berkualitas. Highly recommended!' }] }">
            <div class="text-center mb-14" data-aos="fade-up"> <span
                    class="text-green-400 font-semibold text-sm uppercase tracking-wider">Testimonial</span>
            ...
            <div class="relative max-w-3xl mx-auto"> <template x-for="(t, i) in testimonials" :key="i">
                ...
                <div class="flex items-center justify-center gap-3 mt-8"> <template x-for="(t, i) in testimonials"
                        :key="i"> <button @click="active = i"
                            class="w-3 h-3 rounded-full transition-all duration-300"
                            :class="active === i ? 'bg-green-500 w-8' : 'bg-slate-600 hover:bg-slate-500'"
                            :aria-label="'Testimonial ' + (i + 1)"></button>
                    </template> </div> <button @click="active = (active - 1 + testimonials.length) % testimonials.length" aria-label="Previous testimonial"
                    ...
                    </svg> </button> <button @click="active = (active + 1) % testimonials.length" aria-label="Next testimonial"
    ...
    </section> {{-- Portfolio Highlights --}} @if (isset($latestPortfolios) && $latestPortfolios->isNotEmpty())
        <section class="py-20 lg:py-28">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10" data-aos="fade-up">
                    <div> <span class="text-green-600 font-semibold text-sm uppercase tracking-wider">Karya
                            Terbaru</span>
                        <h2 class="section-title mt-1 ">Portofolio Terkini</h2>
                        <p class="section-subtitle text-slate-500 ">Proyek reklame yang telah kami kerjakan.</p>
                    </div> <a href="{{ route('portofolio.index') }}"
                        class="hidden sm:inline-flex items-center gap-1 text-green-600 hover:text-green-700 font-semibold text-sm transition-colors mt-2">
                        Lihat Semua <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7">
                            </path>
                        ...
                        <article data-aos="fade-up" data-aos-delay="{{ $index * 100 }}"
                            ...
                            <div class="relative aspect-[4/3] overflow-hidden">
                                @if ($portfolio->thumbnail)
                                    <img src="{{ Storage::url($portfolio->thumbnail) }}" alt="{{ $portfolio->title }}" width="400" height="300"
                                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                                    <div class="w-full h-full bg-slate-100 flex items-center justify-center"> <svg
                                            class="w-12 h-12 text-slate-300" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z">
                                            </path>
                                        </svg> </div>
                                    @endif @if ($portfolio->category)
                                        <div class="absolute top-3 left-3"> <span
                            ...
                            <div class="p-5">
                                <h3 class="font-bold text-slate-900 text-lg leading-tight mb-1"> <a
                                        href="{{ route('portofolio.show', $portfolio) }}"
    ...
    @endif {{-- Area / Coverage --}} <section class="bg-slate-50 py-20 lg:py-28">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-14" data-aos="fade-up"> <span
                    class="text-green-600 font-semibold text-sm uppercase tracking-wider">Jangkauan</span>
                <h2 class="section-title mt-2">Area Layanan</h2>
                <p class="section-subtitle mx-auto">Kami melayani dua area utama.</p>
            </div>
            <div class="flex flex-wrap justify-center gap-3" data-aos="fade-up"> <a
                    href="{{ route('catalog.index', ['area' => 'jabodetabek']) }}"
                    ...
                    Jabodetabek </a> <a href="{{ route('catalog.index', ['area' => 'luar_jabodetabek']) }}"
                    class="px-5 py-3 bg-white hover:bg-green-50 border border-slate-200 hover:border-green-200 rounded-xl text-slate-700 hover:text-green-700 font-medium text-sm transition-all shadow-sm hover:shadow-md">
                    Luar Jabodetabek </a> <a href="{{ route('catalog.index') }}"
    ...
    </section> {{-- Latest Posts --}} @if ($latestPosts->isNotEmpty())
        <section class="py-20 lg:py-28">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-10" data-aos="fade-up">
                    <div> <span class="text-green-600 font-semibold text-sm uppercase tracking-wider">Update</span>
                        <h2 class="section-title mt-1 ">Berita & Artikel</h2>
                        <p class="section-subtitle text-slate-500 ">Informasi terbaru seputar dunia reklame.</p>
                    </div> <a href="{{ route('posts.index') }}"
                        class="hidden sm:inline-flex items-center gap-1 text-green-600 hover:text-green-700 font-semibold text-sm transition-colors mt-2">
                        Lihat Semua <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7">
                            </path>
                        ...
                        <article data-aos="fade-up" data-aos-delay="{{ $index * 100 }}"
                            ...
                            <div class="relative aspect-[4/3] overflow-hidden">
                                @if ($post->featured_image)
                                    <img src="{{ Storage::url($post->featured_image) }}" alt="{{ $post->title }}" width="400" height="300"
                                        class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @else
                                    <div class="w-full h-full bg-slate-100 flex items-center justify-center"> <svg
                                            class="w-12 h-12 text-slate-300" fill="none" stroke="currentColor"
                            ...
                            <div class="p-5">
                                <h3 class="font-bold text-slate-900 text-lg leading-tight mb-2"> <a
    ...
    @endif {{-- Trust Badges --}} <section class="bg-white border-y border-slate-100 py-16"
        data-aos="fade-up">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <p class="text-center text-xs text-slate-400 uppercase tracking-widest font-bold mb-12"> Dipercaya
                oleh perusahaan ternama </p>
            <div class="flex flex-wrap items-center justify-center gap-6 md:gap-10"> {{-- Item Logo --}}
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/garuda-indonesia.png') }}" alt="Garuda Indonesia" width="160" height="80"
                ...
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/authenticity.png') }}" alt="Authenticity" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/gojek.png') }}" alt="Gojek" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/bank-bjb.png') }}" alt="Bank BJB" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/vivo.png') }}" alt="Vivo" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/astra-honda-motor.png') }}" alt="Astra Honda Motor" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/MR-DIY.png') }}" alt="MR DIY" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/fore.png') }}" alt="Fore Coffee" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
                <div class="w-32 h-16 md:w-40 md:h-20 flex items-center justify-center p-2 group"> <img
                        src="{{ asset('images/ms-beauty.png') }}" alt="MS Beauty" width="160" height="80"
                        class="max-h-full max-w-full object-contain filter grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300">
                </div>
            </div>
        </div>
    </section> {{-- CTA Section --}} <section
        class="bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 py-20 lg:py-28 relative overflow-hidden">
        <div class="absolute inset-0 opacity-10">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative" data-aos="fade-up">
            ...
            <p class="text-slate-400 text-lg mb-10 max-w-2xl mx-auto">Konsultasikan kebutuhan reklame Anda
                dengan tim profesional kami. Gratis!</p>
            <div class="flex flex-wrap justify-center gap-4"> <a href="{{ route('inquiry.create') }}"
                    ...
                    Konsultasi Gratis <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                    </svg> </a> <a href="{{ route('catalog.index') }}"
