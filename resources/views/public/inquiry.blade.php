...
@section('content')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="sender_name" value="{{ old('sender_name') }}" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all bg-white ">
                        @error('sender_name')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="message" rows="5" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all bg-white ">{{ old('message') }}</textarea>
                        @error('message')
                            <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            {{-- Card Telepon --}}
            <div
                class="bg-white rounded-xl border border-slate-200 p-6 text-center shadow-sm hover:shadow-md transition-shadow">
                ...
                <p class="text-sm text-slate-500">{{ setting('phone', '+62 812-3456-7890') }}</p>
            </div>
    ...
    <section class="w-full bg-slate-50 border-t border-slate-200 py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div
                class="w-full overflow-hidden shadow-md rounded-2xl border border-slate-200/80 bg-white transition-all duration-300 hover:shadow-lg">

                @if (!setting('map_iframe', 'contact.map_iframe'))
                    <div class="flex flex-col items-center justify-center h-[350px] p-6 text-center">
                        <div
                        ...
                        <p class="text-sm font-medium text-slate-500">Peta Lokasi Belum Tersedia</p>
                        <p class="text-xs text-slate-400 mt-1 max-w-xs">Iframe Google Maps belum dikonfigurasi pada
                            pengaturan kontak backend.</p>
                    </div>
                @else
                    <div
                        class="w-full h-[350px] sm:h-[400px] md:h-[450px] lg:h-[480px] [&_iframe]:w-full [&_iframe]:h-full [&_iframe]:border-0 block vertical-align-middle">
