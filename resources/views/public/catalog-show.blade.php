    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
            <div class="grid grid-cols-1 lg:grid-cols-2">
                {{-- Image --}}
                <div class="relative aspect-[4/3] lg:aspect-auto bg-slate-100">
                    @if ($point->thumbnail)
                ...
                <div class="p-8 lg:p-10 flex flex-col">
                    <div class="mb-2">
                        @if ($point->category)
                            <span
                                class="text-xs font-semibold text-green-600 bg-green-50 px-2.5 py-1 rounded-full">{{ $point->category->name }}</span>
                        @endif
                    </div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-slate-900 mb-4">{{ $point->title }}</h1>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-8">
                        @if ($point->size_dimension)
                            <div class="bg-slate-50 rounded-xl p-4">
                                <p class="text-xs text-slate-500 mb-1">Ukuran</p>
                                <p class="font-semibold text-slate-900">{{ $point->size_dimension }}</p>
                            </div>
                        @endif
                        @if ($point->orientation)
                            <div class="bg-slate-50 rounded-xl p-4">
                                <p class="text-xs text-slate-500 mb-1">Orientasi</p>
                                <p class="font-semibold text-slate-900">{{ ucfirst($point->orientation) }}</p>
                            </div>
                        @endif
                        @if ($point->light_type)
                            <div class="bg-slate-50 rounded-xl p-4">
                                <p class="text-xs text-slate-500 mb-1">Pencahayaan</p>
                                <p class="font-semibold text-slate-900">{{ $point->light_type }}</p>
                            </div>
                        @endif
                        @if ($point->lat && $point->long)
                            <div class="bg-slate-50 rounded-xl p-4">
                                <p class="text-xs text-slate-500 mb-1">Koordinat</p>
                                <p class="font-semibold text-slate-900 text-sm">{{ $point->lat }}, {{ $point->long }}
                                </p>
                            </div>
                        @endif
                    </div>
                    ...
                    @if ($point->description)
                        <div class="mb-8">
                            <h2 class="font-semibold text-slate-900 mb-2">Deskripsi</h2>
                            <p class="text-slate-600 text-sm leading-relaxed">{{ $point->description }}</p>
                        </div>
                    @endif

                    <div class="mt-auto pt-6 border-t border-slate-200">
                        <div class="flex items-baseline gap-2 mb-5">
                            <p class="text-sm text-slate-500 font-medium">Harga</p>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            ...
                            <a href="{{ $whatsappLink }}" target="_blank" rel="noopener noreferrer"
                                class="flex-1 inline-flex items-center justify-center gap-2 border-2 border-green-500 text-green-600 hover:bg-green-50 px-6 py-3.5 rounded-xl font-semibold transition-all duration-300">
        ...
        @if ($point->gallery)
            <div class="mt-8" x-data="{ lightbox: null }">
                <h2 class="text-xl font-bold text-slate-900 mb-4">Galeri</h2>
                ...
                <div x-show="lightbox" x-transition.opacity
                    class="fixed inset-0 z-[60] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4"
                    @click="lightbox = null">
        ...
        @if ($relatedPoints->isNotEmpty())
            <div class="mt-16 mb-12">
                <h2 class="text-xl font-bold text-slate-900 mb-6">Titik Reklame Lainnya di {{ $point->city }}</h2>
