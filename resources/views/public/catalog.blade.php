...
@section('content')
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {{-- Filters --}}
        <form method="GET" class="bg-white rounded-2xl border border-slate-200 shadow-sm mb-8" x-data="{ showFilters: false }">
            <div class="p-5 border-b border-slate-100 flex items-center justify-between">
                ...
                <button type="button" @click="showFilters = !showFilters"
                    class="lg:hidden text-sm text-green-600 hover:text-green-700 font-medium flex items-center gap-1">
            ...
            <div class="p-5" :class="{ 'hidden lg:block': !showFilters }">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Cari</label>
                        <div class="relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}"
                                ...
                                class="w-full pl-9 pr-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Kota</label>
                        <select name="city"
                            class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 outline-none transition-all">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Orientasi</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Pencahayaan</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Min. Harga</label>
                        <div class="relative">
                            <span
                                class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-medium">Rp</span>
                            <input type="number" name="price_min" value="{{ request('price_min') }}" placeholder="0"
                                class="w-full pl-9 pr-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Max. Harga</label>
                        <div class="relative">
                            <span
                                class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-medium">Rp</span>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Urutkan</label>
                        <select name="sort"
                            class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 outline-none transition-all">
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    @if (request()->hasAny(['search', 'city', 'category', 'orientation', 'light_type', 'price_min', 'price_max', 'sort']))
                        <a href="{{ route('catalog.index') }}"
                            class="inline-flex items-center gap-2 px-6 py-2.5 border border-slate-300 text-slate-600 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-all">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex items-center justify-between mb-6">
            <p class="text-sm text-slate-500">
                Menampilkan <span class="font-semibold text-slate-900">{{ $points->firstItem() ?? 0 }}</span>-<span
                    class="font-semibold text-slate-900">{{ $points->lastItem() ?? 0 }}</span>
                dari <span class="font-semibold text-slate-900">{{ $points->total() }}</span> titik reklame
            </p>
        </div>
        ...
        @if ($points->isEmpty())
            <div class="text-center py-20">
                ...
                <h3 class="text-xl font-semibold text-slate-900 mb-2">Tidak Ditemukan</h3>
                <p class="text-slate-500">Tidak ada titik reklame yang sesuai dengan filter Anda.</p>
            </div>
        @else
