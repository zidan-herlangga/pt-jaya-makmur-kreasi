<footer class="bg-slate-900 text-slate-400 relative"> {{-- Decorative top border --}} <div
        class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-green-500 via-green-400 to-green-500"></div>
    {{-- Newsletter --}} <div class="border-b border-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col lg:flex-row items-center justify-between gap-6">
                ...
                <form class="flex w-full lg:w-auto gap-3" x-data="{ email: '', loading: false, message: '', error: '' }"
                    @submit.prevent=" loading = true; error = ''; message = ''; fetch('{{ route('newsletter.subscribe') }}', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: JSON.stringify({ email }) }).then(r => r.json()).then(d => { if (d.message) { message = d.message; email = ''; setTimeout(() => message = '', 4000) } }).catch(e => error = 'Terjadi kesalahan. Coba lagi.') .finally(() => loading = false) ">
                    <div class="relative flex-1 lg:w-72"> <input type="email" x-model="email" required
                        ...
                        class="px-6 py-3 bg-green-700 hover:bg-green-600 disabled:bg-green-700/50 text-white rounded-xl text-sm font-semibold transition-all whitespace-nowrap shadow-lg shadow-green-700/20">
                        <span x-show="!loading">Berlangganan</span> <span x-show="loading"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10"> {{-- Company Info --}} <div
                ...
                <p class="text-sm leading-relaxed text-slate-400">
                    {{ setting('site_description', 'Perusahaan penyedia jasa reklame dan billboard profesional. Melayani pemasangan media promosi luar ruang di berbagai kota besar Indonesia dengan kualitas terbaik.') }}
                        ...
                        aria-label="Facebook" target="_blank" rel="noopener"> <svg class="w-4 h-4" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path
                                d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg> </a> <a href="{{ setting('instagram_url', '#') }}"
                        ...
                        aria-label="Instagram" target="_blank" rel="noopener"> <svg class="w-4 h-4" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path
                        ...
                        </svg> </a> <a href="{{ setting('linkedin_url', '#') }}"
                        ...
                        aria-label="LinkedIn" target="_blank" rel="noopener"> <svg class="w-4 h-4" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path
                ...
                <ul class="space-y-4">
                    <li class="flex items-start gap-3"> <svg class="w-5 h-5 text-green-400 mt-0.5 shrink-0"
                    ...
                    <li class="flex items-center gap-3"> <svg class="w-5 h-5 text-green-400 shrink-0" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                        </svg> <span class="text-sm text-slate-400">{{ setting('phone', '+62 812-3456-7890') }}</span>
                    </li>
                    <li class="flex items-center gap-3"> <svg class="w-5 h-5 text-green-400 shrink-0" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
