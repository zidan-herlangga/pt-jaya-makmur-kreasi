    ...
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <form method="GET" x-data="{ search: '{{ request('search') }}', status: '{{ request('status') }}', category_id: '{{ request('category_id') }}' }" x-ref="filterForm" class="flex flex-wrap gap-3">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" x-model="search" @input.debounce.500ms="$refs.filterForm.submit()" placeholder="Cari berita..."
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden"
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($posts as $post)
                        <tr class="hover:bg-slate-50 transition-colors" :class="selectedIds.includes('{{ $post->id }}') && 'bg-green-50'">
                            <td class="px-6 py-4">
                                <input type="checkbox" value="{{ $post->id }}" x-model="selectedIds" class="row-checkbox rounded border-slate-300 text-green-600 focus:ring-green-500">
                            </td>
                            <td class="px-6 py-4">
        ...
        <div x-show="showBulkDeleteModal" x-cloak
            ...
            <div class="relative bg-white rounded-xl shadow-xl p-6 max-w-md w-full mx-4">
                ...
                <p class="text-sm text-slate-500 mt-2" x-text="'Anda akan menghapus ' + count + ' berita. Tindakan ini tidak dapat dibatalkan.'"></p>
