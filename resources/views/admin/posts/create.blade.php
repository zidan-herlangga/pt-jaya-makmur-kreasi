@extends('layouts.admin', ['seo' => $seo ?? ['title' => 'Tulis Berita']])
...
<textarea id="editor" name="content_body" rows="16"
    class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none">{{ old('content_body') }}</textarea>
                    @error('content_body')
                        <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Ringkasan (Excerpt)</label>
