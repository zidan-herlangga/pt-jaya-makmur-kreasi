@extends('layouts.admin', ['seo' => $seo ?? ['title' => 'Edit Titik Reklame']])
...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('admin.advertising-points.update', $advertisingPoint->id) }}" method="POST"
            enctype="multipart/form-data" class="bg-white rounded-xl border border-slate-200 p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{ autoSlug: true }">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" x-ref="title" value="{{ old('title', $advertisingPoint->title) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="location_name"
                        value="{{ old('location_name', $advertisingPoint->location_name) }}"
                        class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Kota</label>
                    <input type="text" name="city" value="{{ old('city', $advertisingPoint->city) }}"
                        class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                        class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none">{{ old('description', $advertisingPoint->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    @if ($advertisingPoint->thumbnail)
                        <div class="mb-2">
                            <img src="{{ Storage::url($advertisingPoint->thumbnail) }}"
                ...
                <div>
                    ...
                    @if ($advertisingPoint->og_image)
                        <div class="mb-2">
                            <img src="{{ Storage::url($advertisingPoint->og_image) }}"
                                class="w-32 h-24 object-cover rounded-lg">
                        </div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    @if (is_array($advertisingPoint->gallery) || is_object($advertisingPoint->gallery))
                        @foreach ($advertisingPoint->gallery as $img)
                            @if (is_string($img))
                                <img src="{{ Storage::url($img) }}" class="w-20 h-16 object-cover rounded-lg">
                            @endif
                        @endforeach
                    @else
